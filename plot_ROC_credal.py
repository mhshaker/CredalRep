...
for data in data_list:
    ...
    for job in jobs:
        ...
        for mode_index, mode in enumerate(modes):
            ...
            acc_rej_area = metrics.auc(steps[:-1], avg_acc[:-1])
            # print(steps)
            # exit()
            # print(">>>> ", acc_rej_area)
            ...
            if single_plot:
                ...
                if mode == "t":
                    if "Levi-GH" in legend:
                        # Exception for AU
                        axs[1].plot(steps, avg_acc, linestyle='--', color=color, label="AU" + f"(ROC {roc_avg_area:.2f} AR {acc_rej_area:.2f})")
                        axs[1].legend()
                ...
                if len(plot_legend)>0:
                    axs[mode_index].plot(steps, avg_acc, linestyle=linestyle, color=color, label=plot_legend + f"(ROC {roc_avg_area:.2f} AR {acc_rej_area:.2f})")
